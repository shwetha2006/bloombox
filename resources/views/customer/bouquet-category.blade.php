{{-- resources/views/customer/bouquet-category.blade.php --}}
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <title>BloomBox - {{ $category->name }}</title>
...
<body class="bg-black text-white">
    ...
    <main class="container mx-auto py-10 px-4">
        <h1 class="text-3xl font-bold mb-6 text-center text-yellow-400">{{ $category->name }} Bouquets</h1>
        ...
        <div class="flex items-center gap-4 mb-6">
            ...
            <div class="relative">
                ...
                <div id="price-dropdown" class="hidden absolute mt-2 p-4 bg-gray-900 rounded shadow w-64 z-50">
                    <form method="GET" action="{{ route('customer.category-bouquets', $category->id) }}">
                        <label class="block text-yellow-400 font-semibold mb-2">Min Price</label>
                        <input type="number" name="minPrice" value="{{ request('minPrice', 0) }}" 
                               min="0" max="20000" step="500"
                               class="w-full px-3 py-2 rounded text-black mb-4">

                        <label class="block text-yellow-400 font-semibold mb-2">Max Price</label>
